...
class StoreGen(typ: UInt, addr: UInt, dat: UInt, maxSize: Int) {
  ...
  def mask = {
    ...
    for (i <- 0 until log2Up(maxSize)) {
      val upper =
        Mux(addr(i), res, 0.U) | Mux(size >= (i + 1).U, ((BigInt(1) << (1 << i)) - 1).U, 0.U)
...
class AMOALU(
  ...
  val adder_out = {
    ...
    val mask =
      ~(0.U(operandBits.W) +: widths.init.map(w => !io.mask(w / 8 - 1) << (w - 1))).reduce(_ | _)
  ...
  val less = {
    ...
    def isLessUnsigned(x: UInt, y: UInt, n: Int): Bool = {
      if (n == minXLen) x(n - 1, 0) < y(n - 1, 0)
        ...
        x(n - 1, n / 2) < y(n - 1, n / 2) || x(n - 1, n / 2) === y(n - 1, n / 2) && isLessUnsigned(
          x,
          y,
          n / 2
        )
