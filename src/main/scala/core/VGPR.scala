...
class VGPR(
  ...
  val need_forward1 = RegInit(0.B)
  val need_forward2 = RegInit(0.B)
  val forward_data = RegInit(0.U.asTypeOf(Vec(numThreads, UInt(xLen.W))))
  need_forward1 := io.writeback.bits.rd === io.read_req.rs1 && io.writeback.bits.wid === io.read_req.wid
  need_forward2 := io.writeback.bits.rd === io.read_req.rs2 && io.writeback.bits.wid === io.read_req.wid
  forward_data := io.writeback_cmd.bits.data
  ...
  io.read_rsp.rs1_data := Mux(
    raddr_reg === 0.U,
    0.U.asTypeOf(gpr_ram(0).dataOut),
    Mux(need_forward1, forward_data, gpr_ram(rwid_reg).dataOut)
  )
  io.read_rsp.rs2_data := Mux(
    raddr2_reg === 0.U,
    0.U.asTypeOf(gpr_ram(0).dataOut),
    Mux(need_forward2, forward_data, gpr_ram(rwid_reg).dataOut2)
  )
}
